...
const ART_MANIFEST = {
  ...
  // Onboarding wing leads (special_scout / special_end)
  ship_reva:         "sprites/space/ship_reva.png",
...
// Mira terrain tiles always render (not gated by GAMEPLAY_SPRITES).
...
const TERRAIN_TILESETS = ['mira', 'vesper', 'cinder', 'dusk', 'sorn', 'selene'];
...
// Per-world prop packs (earth / barren / lava / ice / desert). Mirror variants
// (*_m) double variety without new art — drawn with flip OR as separate files.
const PROP_SETS = ['mira', 'vesper', 'cinder', 'dusk', 'sorn', 'selene'];
const PROP_VARIANTS = [
...
for (const set of PROP_SETS)
  for (const v of PROP_VARIANTS) {
    ...
    PROP_KEYS.add(k);
    // Horizontal-mirror variant files (pipeline / recolor pass)
    const km = `prop_${set}_${v}_m`;
    ART_MANIFEST[km] = `sprites/${set}/prop_${v}_m.png`;
    PROP_KEYS.add(km);
  }
...
const SLAB_SETS = ['mira', 'vesper', 'cinder', 'dusk', 'sorn', 'selene'];
const SLAB_VARIANTS = [
  'grass_a', 'grass_b', 'grass_c', 'grass_flowers', 'dirt', 'dirt_b', 'stone_path', 'water',
  'soil_tilled', 'soil_seedling', 'soil_harvest',
];

// Large collidable landmarks (optional art; emoji fallback if missing).
// Must stay in the fetch gate (LANDMARK_KEYS) — otherwise GAMEPLAY_SPRITES=false
// deletes them and surface landmarks never load.
const LANDMARK_SETS = ['mira', 'vesper', 'cinder', 'dusk', 'sorn', 'selene'];
const LANDMARK_VARIANTS = ['mesa', 'boulder', 'ridge', 'mountain', 'volcano', 'dune'];
const LANDMARK_KEYS = new Set();
for (const set of LANDMARK_SETS)
  for (const v of LANDMARK_VARIANTS) {
    const k = `landmark_${set}_${v}`;
    ART_MANIFEST[k] = `sprites/${set}/landmark_${v}.png`;
    LANDMARK_KEYS.add(k);
  }

// Per-world FEATURE packs (WORLD_TYPES.md): oriented cliffs + peaks + flora.
// Packs: mira=earth, dusk=ice, cinder=lava, sorn=desert, selene=moon base.
const FEAT_SETS = ['mira', 'dusk', 'cinder', 'sorn', 'selene'];
const FEAT_VARIANTS = [
  'cliff_s', 'cliff_e', 'cliff_n', 'cliff_w',
  'peak', 'boulder', 'outcrop', 'plant_a', 'plant_b', 'accent',
];
const FEAT_KEYS = new Set();
for (const set of FEAT_SETS)
  for (const v of FEAT_VARIANTS) {
    const k = `feat_${set}_${v}`;
    ART_MANIFEST[k] = `sprites/${set}/feat_${v}.png`;
    FEAT_KEYS.add(k);
  }
// Optional dish accent for moon base
ART_MANIFEST['feat_selene_dish'] = 'sprites/selene/feat_dish.png';
FEAT_KEYS.add('feat_selene_dish');
// Large lunar ground plates / crater bowls / cliff walls (authored look, not cell stamps)
const PLATE_KEYS = new Set();
for (let i = 0; i < 4; i++) {
  ART_MANIFEST[`plate_selene_crater_${i}`] = `sprites/selene/plate_crater_${i}.png`;
  ART_MANIFEST[`plate_selene_regolith_${i}`] = `sprites/selene/plate_regolith_${i}.png`;
  PLATE_KEYS.add(`plate_selene_crater_${i}`);
  PLATE_KEYS.add(`plate_selene_regolith_${i}`);
}
for (const f of ['s', 'e', 'n', 'w']) {
  ART_MANIFEST[`plate_selene_cliff_${f}`] = `sprites/selene/plate_cliff_${f}.png`;
  PLATE_KEYS.add(`plate_selene_cliff_${f}`);
}
// Vesper barren → share mira earth features
for (const v of FEAT_VARIANTS) {
  const k = `feat_vesper_${v}`;
  ART_MANIFEST[k] = `sprites/mira/feat_${v}.png`;
  FEAT_KEYS.add(k);
}
...
// Surface explorer avatar — 4 facings × 4 walk frames (clay diorama style).
// Generated sheet: sprites/mira/player_explorer.png (see planet_surface drawPlayer).
// PLAYER_KEYS keeps these out of the GAMEPLAY_SPRITES fetch-gate purge.
const PLAYER_DIRS = ['s', 'e', 'n', 'w'];
const PLAYER_FRAMES = 4;
const PLAYER_KEYS = new Set();
for (const d of PLAYER_DIRS)
  for (let f = 0; f < PLAYER_FRAMES; f++) {
    const k = `player_explorer_${d}_${f}`;
    ART_MANIFEST[k] = `sprites/mira/player_explorer_${d}_${f}.png`;
    PLAYER_KEYS.add(k);
  }
...
const NODE_SETS = ['mira', 'vesper', 'cinder', 'dusk', 'sorn', 'selene'];
const NODE_VARIANTS = [
  'tree_oak_a', 'tree_oak_b', 'tree_palm', 'tree_pine', 'tree_cactus', 'tree_crystal', 'tree_dead',
  'rock_a', 'rock_b', 'rock_crystal', 'rock_ice', 'rock_basalt',
  'berry_bush', 'berry_glow', 'berry_cactus',
];
...
const BLDG_SETS = ['vesper', 'cinder', 'dusk', 'sorn', 'selene'];
...
// Wing companions (always load — used on the live onboarding ladder)
SPACE_CLAY_KEYS.add('ship_reva');
...
// bandwidth on PNGs that can never draw. STORY_KEYS and the Mira ground/building
// sets always render, so they always load.
if (!GAMEPLAY_SPRITES)
  for (const k of Object.keys(ART_MANIFEST))
    if (!STORY_KEYS.has(k) && !MIRA_TERRAIN_KEYS.has(k) && !SLAB_KEYS.has(k) &&
        ...
        !SPACE_CLAY_KEYS.has(k) && !LANDMARK_KEYS.has(k) && !PLAYER_KEYS.has(k) &&
        !FEAT_KEYS.has(k) && !PLATE_KEYS.has(k))
...
const ART = {
  ...
  drawSlab(ctx, key, sx, sy, hw, hh, tint) {
    ...
    if (oc === undefined) {
      ...
      if (tint) {
        o.globalCompositeOperation = 'multiply'; o.globalAlpha = 0.58;
  ...
  _propCache: new Map(),
  // flipX: mirror horizontally. Prefers prop_*_m.png if loaded, else runtime flip.
  drawProp(ctx, key, sx, sy, wPx, res, flipX) {
    let useKey = key;
    let runtimeFlip = !!flipX;
    if (flipX && this.get(key + '_m')) {
      useKey = key + '_m';
      runtimeFlip = false; // file is already mirrored
    }
    let oc = this._propCache.get(useKey);
    if (oc === undefined) {
      const src = this.get(useKey) || this.get(key);
      if (!src) return false;
      if (!this.get(useKey)) { useKey = key; runtimeFlip = !!flipX; }
      // Larger default cache: full-tile grass carpet needs crisp edges (was 96)
      const P = res || 192, s = P / Math.max(src.naturalWidth, src.naturalHeight);
    ...
    if (runtimeFlip) {
      ...
      ctx.translate(sx, sy);
      ctx.scale(-1, 1);
      ctx.drawImage(oc, -wPx / 2, -hPx, wPx, hPx);
      ctx.restore();
    } else {
