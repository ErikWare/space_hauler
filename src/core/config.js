...
const CONFIG = {
  ...
  // SHIPS, never modules).
  //
  // PROGRESSION (slots drive capability):
  //   equipSlots  — module rack size (3 → 6). HUD skills 1..N follow the live rack.
  //   escortSlots — drone wing that flies with you (1 → 6). Hangar can still own
  //                 up to DRONES.ownedMax; only escortSlots leave the bay in combat.
  // Starter is intentionally weak (3 mods / 1 drone). Each line's MK III can push
  // deep null-sec; only Nox Eclipse has the full 6-drone crown deck.
  //
  // cost:null = starter; buyable hulls at SHIPS market. unlock = outposts OR
  // maxDangerReached (either condition). Four LINES (shipLines carousel):
  //   hauler — freighter progression (balanced)
  //   krag   — armor brawlers + mid wing
  //   vex    — gun platforms + mid wing
  //   nox    — tank carriers + largest wing
  // baseShip.mass feeds ship↔rock/junk collision; faction hulls may cost exotic ore.
  hulls: {
    // ---- HAULER LINE ----
    vulture: {
      name: "Vulture Tug", tier: "STARTER", line: "hauler",
      desc: "starter tug — 3 module racks, one drone hardpoint. Learn the lanes before you buy a deck.",
      cost: null, baseTows: 2, equipSlots: 3, escortSlots: 1,
      baseShip: {
        shieldMax: 1100, shieldRegen: 55, shieldDelay: 3.2,
        armorMax: 900, armorRepair: 0,
        hullMax: 700,
        res: { shield: 0, armor: 0.12, hull: 0 },
        fuelMax: 1200, solarRegen: 1.8,
        thrust: 105, turnSpeed: 105,
        scanRange: 850,
        tractorRange: 550, tractorStr: 0.9,
        fuelCostK: 1,
        weaponDmg: 8, fireRate: 1, weaponRange: 0,
        mass: 1.8,
      },
    },
    atlas: {
      name: "Atlas Freighter", tier: "MID-TIER", line: "hauler",
      desc: "workhorse freighter — 4 racks, two-drone wing, big tows. Respectable mid-game hull.",
      cost: { credits: 45000 }, baseTows: 4, equipSlots: 4, escortSlots: 2,
      unlock: { outposts: 2, danger: 3 },
      baseShip: {
        shieldMax: 1900, shieldRegen: 75, shieldDelay: 3.0,
        armorMax: 1700, armorRepair: 0,
        hullMax: 1400,
        res: { shield: 0, armor: 0.20, hull: 0 },
        fuelMax: 1600, solarRegen: 2.4,
        thrust: 92, turnSpeed: 92,
        scanRange: 1000,
        tractorRange: 700, tractorStr: 1.4,
        fuelCostK: 1,
        weaponDmg: 11, fireRate: 1,
        mass: 3.2,
      },
    },
    aegis: {
      name: "Aegis Warhauler", tier: "HEAVY", line: "hauler",
      desc: "null-sec hauler — 5 racks, four-drone wing. Can push high ranks; not quite a crown deck.",
      cost: { credits: 220000 }, baseTows: 4, equipSlots: 5, escortSlots: 4,
      unlock: { outposts: 8, danger: 6 },
      baseShip: {
        shieldMax: 3000, shieldRegen: 100, shieldDelay: 2.5,
        armorMax: 2800, armorRepair: 1,
        hullMax: 2200,
        res: { shield: 0.08, armor: 0.28, hull: 0.05 },
        fuelMax: 1700, solarRegen: 2.4,
        thrust: 118, turnSpeed: 112,
        scanRange: 1150,
        tractorRange: 650, tractorStr: 1.3,
        fuelCostK: 1,
        weaponDmg: 15, fireRate: 1.05,
        mass: 4.5,
      },
    },
    // ---- KRAG battlecruisers (armor + mass; wing grows with tier) ----
    krag_ironclad: {
      name: "Krag Ironclad", tier: "BC MK I", line: "krag",
      desc: "riveted brawler — 3 racks, one wing drone. Armor first, everything else second.",
      cost: { credits: 35000 }, baseTows: 3, equipSlots: 3, escortSlots: 1,
      unlock: { outposts: 2, danger: 3 },
      baseShip: {
        shieldMax: 1500, shieldRegen: 65, shieldDelay: 3.2,
        armorMax: 2600, armorRepair: 0,
        hullMax: 1600,
        res: { shield: 0, armor: 0.32, hull: 0 },
    ...
    krag_warbarge: {
      ...
      desc: "rolling fortress — 4 racks, three-drone wing. Hauls and holds.",
      cost: { credits: 120000, ore: { iridium: 4 } }, baseTows: 4, equipSlots: 4, escortSlots: 3,
      unlock: { outposts: 5, danger: 5 },
      baseShip: {
        shieldMax: 2100, shieldRegen: 80, shieldDelay: 3.0,
        armorMax: 3600, armorRepair: 1,
        hullMax: 2300,
        res: { shield: 0.02, armor: 0.38, hull: 0.06 },
        fuelMax: 1750, solarRegen: 2.5,
        thrust: 100, turnSpeed: 90,
        scanRange: 1000,
        tractorRange: 720, tractorStr: 1.45,
        fuelCostK: 1,
        weaponDmg: 16, fireRate: 1,
        mass: 6.5,
      },
    },
    krag_dreadnought: {
      name: "Krag Dreadnought", tier: "BC MK III", line: "krag",
      desc: "flying fortress — 5 racks, four-drone wing. Endgame armor path; crown still wants Eclipse.",
      cost: { credits: 280000, ore: { iridium: 8, cryonite: 4 } }, baseTows: 5, equipSlots: 5, escortSlots: 4,
      unlock: { outposts: 10, danger: 7 },
      baseShip: {
        shieldMax: 3000, shieldRegen: 95, shieldDelay: 2.8,
        armorMax: 5200, armorRepair: 3,
        hullMax: 3200,
        res: { shield: 0.08, armor: 0.45, hull: 0.12 },
        fuelMax: 1900, solarRegen: 2.8,
        thrust: 108, turnSpeed: 95,
        scanRange: 1100,
        tractorRange: 780, tractorStr: 1.7,
        fuelCostK: 1,
        weaponDmg: 20, fireRate: 1.05,
        mass: 9,
      },
    },
    // ---- VEX destroyers (guns + speed; wing grows with tier) ----
    vex_lance: {
      name: "Vex Lance", tier: "DD MK I", line: "vex",
      desc: "knife destroyer — 3 racks, one wing drone. First to the fight.",
      cost: { credits: 70000 }, baseTows: 3, equipSlots: 3, escortSlots: 1,
      unlock: { outposts: 3, danger: 4 },
      baseShip: {
        shieldMax: 2100, shieldRegen: 90, shieldDelay: 2.7,
        armorMax: 1500, armorRepair: 0,
        hullMax: 1250,
        res: { shield: 0.06, armor: 0.18, hull: 0 },
        fuelMax: 1500, solarRegen: 2,
        thrust: 135, turnSpeed: 128,
        scanRange: 1250,
        tractorRange: 600, tractorStr: 1,
        fuelCostK: 1,
        weaponDmg: 18, fireRate: 1.2,
        mass: 3.4,
      },
    },
    vex_saber: {
      name: "Vex Saber", tier: "DD MK II", line: "vex",
      desc: "twin spinals — 4 racks, three-drone wing. Executioner's mid-tier.",
      cost: { credits: 230000, ore: { iridium: 5, cryonite: 3 } }, baseTows: 3, equipSlots: 4, escortSlots: 3,
      unlock: { outposts: 8, danger: 6 },
      baseShip: {
        shieldMax: 3000, shieldRegen: 105, shieldDelay: 2.5,
        armorMax: 2100, armorRepair: 0,
        hullMax: 1750,
        res: { shield: 0.12, armor: 0.24, hull: 0 },
        fuelMax: 1650, solarRegen: 2.3,
        thrust: 145, turnSpeed: 132,
        scanRange: 1350,
        tractorRange: 620, tractorStr: 1.1,
        fuelCostK: 1,
        weaponDmg: 24, fireRate: 1.3,
        mass: 5.2,
      },
    },
    vex_executor: {
      name: "Vex Executor", tier: "DD MK III", line: "vex",
      desc: "siege destroyer — 5 racks, four-drone wing. Glass-cannon endgame; not a tank, still clears high ranks.",
      cost: { credits: 550000, ore: { cryonite: 7, solarite: 4 } }, baseTows: 4, equipSlots: 5, escortSlots: 4,
      unlock: { outposts: 13, danger: 8 },
      baseShip: {
        shieldMax: 4000, shieldRegen: 125, shieldDelay: 2.3,
        armorMax: 2600, armorRepair: 0,
        hullMax: 2100,
        res: { shield: 0.16, armor: 0.28, hull: 0.05 },
        fuelMax: 1750, solarRegen: 2.5,
        thrust: 155, turnSpeed: 138,
        scanRange: 1500,
        tractorRange: 660, tractorStr: 1.2,
        fuelCostK: 1,
        weaponDmg: 32, fireRate: 1.4,
        mass: 7.2,
      },
    },
    // ---- NOX carriers (tank + largest wing; soft main guns) ----
    nox_veil: {
      name: "Nox Veil", tier: "CV MK I", line: "nox",
      desc: "escort carrier — 4 racks, three-drone wing. Learn the tank-and-wing pattern.",
      cost: { credits: 160000, ore: { iridium: 5 } }, baseTows: 4, equipSlots: 4, escortSlots: 3,
      unlock: { outposts: 5, danger: 5 },
      baseShip: {
        shieldMax: 3600, shieldRegen: 115, shieldDelay: 2.4,
        armorMax: 2500, armorRepair: 1,
        hullMax: 2300,
        res: { shield: 0.14, armor: 0.24, hull: 0.05 },
        fuelMax: 1850, solarRegen: 2.9,
        thrust: 105, turnSpeed: 85,
        scanRange: 1450,
        tractorRange: 720, tractorStr: 1.35,
        fuelCostK: 1,
        weaponDmg: 9, fireRate: 0.9,
        mass: 7.5,
      },
    },
    nox_umbra: {
      name: "Nox Umbra", tier: "CV MK II", line: "nox",
      desc: "fleet carrier — 5 racks, five-drone wing. High-rank siege tank; one step below crown.",
      cost: { credits: 400000, ore: { solarite: 5, cryonite: 5 } }, baseTows: 5, equipSlots: 5, escortSlots: 5,
      unlock: { outposts: 10, danger: 7 },
      baseShip: {
        shieldMax: 5200, shieldRegen: 150, shieldDelay: 2.15,
        armorMax: 3800, armorRepair: 2,
        hullMax: 3100,
        res: { shield: 0.20, armor: 0.32, hull: 0.08 },
        fuelMax: 2100, solarRegen: 3.3,
        thrust: 108, turnSpeed: 88,
        scanRange: 1650,
        tractorRange: 760, tractorStr: 1.55,
        fuelCostK: 1,
        weaponDmg: 10, fireRate: 0.9,
        mass: 11,
      },
    },
    nox_eclipse: {
      name: "Nox Eclipse", tier: "CV MK III", line: "nox",
      desc: "cathedral supercarrier — 6 racks, SIX-drone wing. Only deck that clears station crown contracts.",
      cost: { credits: 850000, ore: { voidium: 5, solarite: 6 } }, baseTows: 6, equipSlots: 6, escortSlots: 6,
      unlock: { outposts: 15, danger: 8 },
      baseShip: {
        shieldMax: 7200, shieldRegen: 180, shieldDelay: 1.95,
        armorMax: 5200, armorRepair: 5,
        hullMax: 4200,
        res: { shield: 0.28, armor: 0.38, hull: 0.14 },
        fuelMax: 2500, solarRegen: 4.0,
        thrust: 110, turnSpeed: 90,
        scanRange: 1900,
        tractorRange: 820, tractorStr: 1.9,
        fuelCostK: 1,
        weaponDmg: 10, fireRate: 0.85,
        mass: 15,
      },
    },
  },
  // SHIPS-market carousel line order + labels (ships.js).
  shipLines: {
    hauler: { name: "Hauler Guild",  sub: "freighter-tug line · slots & wing scale with tier", col: "#57d1c9" },
    krag:   { name: "Krag Foundry",  sub: "battlecruisers · armor tanks + mid wing",           col: "#ff9a5a" },
    vex:    { name: "Vex Arsenal",   sub: "destroyers · firepower + mid wing",                 col: "#ff6b6b" },
    nox:    { name: "Nox Grove",     sub: "carriers · max wing + fortress plate (crown deck)", col: "#c9b8ff" },
  },
